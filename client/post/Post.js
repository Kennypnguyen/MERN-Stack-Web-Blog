...
// Post component that will render the post details (header, content, actions, comments)
...
export default function Post (props){
  ...
  // Check if a post has been liked and counting likes
  const [values, setValues] = useState({
    like: checkLike(props.post.likes),
    likes: props.post.likes.length,
    comments: props.post.comments
  })
  // Handles the clicks on the like and unlike button
  const clickLike = () => {
  ...
  // Update the comments of the post when a comment is added or deleted
  const updateComments = (comments) => {
    setValues({...values, comments: comments})
  }
  // DeletePost method is called when delete button is clicked 
  const deletePost = () => {   
    remove({
